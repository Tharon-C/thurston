...
class KLNProduct {
	...
	function get_product() {
		...
		//$options = get_post_meta($this->id, "product_spec");
		...
		if ( $attachments ) {
			...
			$product .= ($meta['gsa'] == true) ? '<img src="'.$upload_dir['url'].'/gsa-check.png" class="gsa-logo">' : '';
		...
		$product .='<article id="content-area post-'.$this->id.'" '.get_post_class($this->id).
		...
		$product .='</tbody>
				...
				<a class = "button-1" href="'.content_url().'/uploads/'.preg_replace('#.*?\/uploads\/#', '', $meta['spec_sheet_url'][0]).'">Spec Sheet</a>
			</div>';
		...
		if ($laminate_results) { 
			$product .=		'<div class="laminate-selected">
								<div class="laminate-selected-in selected-in"></div>
								<div class="laminate-selected-out selected-out"></div>
	...
	'</div>';
		...
		if (!$paint_results && !$laminate_results && !$door_results) {
	...
	function get_swatches($taxonomy) {		
		...
		if ($results) {	
			$swatch .= '<div class="options-div">'.
				'<span><strong>'.strtoupper($taxonomy).'</strong></span><ul class="swatches-ul clear">';
