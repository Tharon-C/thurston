...
function display_custom_product_images($p = NULL) {
	if ($p)
		$post = $p;
	...
	$return .= '<div id="dropzone-element" class="featured-image-wrap"><img class="featured-product-image">Drop Files or Click Here</div>';
	$return .= '</div>';
	return $return;	
}
...
function upload_new_custom_product_file() {
	...
	if (!file_exists($pathname)) {
		mkdir($pathname);
		if (!file_exists($pathname)) {
			echo json_encode(["message"=>"Could Not Create 'custom_products' Directory--Check File Permissions in Your Uploads Folder", "success"=>false]);
			exit;			
		}
	}
		...
		if ($file_has_been_moved) {
			...
			if($metadata_has_been_set) {
				$message = [
					...
					"temp_stored_in" => $_FILES["file"]["tmp_name"],
					"stored_in" => $filepath
				];
				$success = true;
			} else {
  		...
  		$message = ["error" => "Invalid File Type!",
					...
					"stored_in" => $_FILES["file"]["tmp_name"]
					];
		$success = false;
  	}
	echo json_encode(["message"=>$message, "success"=>$success, "post" => $post, 'post_id' => $_GET['post_id']]);
	exit;
}
